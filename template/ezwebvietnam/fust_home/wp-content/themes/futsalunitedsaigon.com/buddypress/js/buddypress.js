...
jq(document).ready( function() {
		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				if ( 0 == jq("ul.activity-list").length ) {
					...
					jq("#item-body div.activity").append( '<ul id="activity-stream" class="activity-list item-list">' );
				}
	...
	jq('#item-body div.activity').on( 'click', function(event) {
		...
		if ( target.hasClass('delete-activity') ) {
			var li        = target.parents('#item-body div.activity ul li');
...
function bp_activity_request(scope, filter) {
	...
	function(response)
	{
		...
		jq('#item-body div.activity').fadeOut( 100, function() {
